...
export const Timeline = ({ data }) => {
  ...
  return (
    <div className="w-full bg-[#000000c4] font-sans md:px-10" ref={containerRef}>
      <div className="max-w-7xl mx-auto py-24 px-4 md:px-8 lg:px-10 text-center">
        <h2 className="text-2xl md:text-5xl mb-4 text-white max-w-4xl font-bold mx-auto">
          Education
        </h2>
        <p className="text-neutral-300 text-sm md:text-lg max-w-sm text-center mx-auto">
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <div className="hidden md:block md:pl-20">
                <h3 className="text-2xl md:text-5xl font-bold text-neutral-500 ">
                  {item.date}
                </h3>
                <h3 className="text-3xl text-neutral-400 font-semibold mt-2">
                  {item.title}
                </h3>
              </div>
            </div>
            ...
            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <div className="md:hidden block text-2xl mb-4 text-left font-bold text-neutral-500">
                <h3>{item.date}</h3>
                <h3 className="text-xl text-neutral-300">{item.title}</h3>
              </div>

              {item.contents.map((content, idx) => (
                <p
                  className="text-neutral-300 text-sm md:text-base font-normal mb-4" 
                  key={idx}
                >
                  {content}
                </p>
